...
想了想这一个相当重要的问题，抽象一下就是，**如何快速地「获取」和「存储」信息？**分解一下，有怎么几个问题需要处理。
	
* 文件存哪里？
* 文件有哪些？
* 如何获取呢？
	
如果，存储空间，无限大且特别便宜，我们就没有这样的困扰。正是资源有限，成本不同，导致我们不得不精打细算。所以我们的电脑有了寄存器、高速缓存、内存、硬盘这样存储结构。
	
展开去讲，就有太多东西可以讲了，甚至是住房问题都可以抽象为这个「获取」与「存储」的问题。这里，**我们只聊聊，如何让我们电脑里的资料井井有条？**
...
* 最好的知识整理是**输出**。(输出是强有力的知识胶水)
* 不要抛弃**纸和笔**。（灵活性强，避免分心，一览无余）
...
2. 知识
	...
	* 存储地点：
		...
		* Calibre: 所有书都存储在这里。重要便于Spotlight搜索。
		* iBook: 结构化的书单存储在这里。便于阅读。
		* Zotero: 所有论文都存储于此。
		* Evernote: 大脑的硬盘。所有网络文章，都存储于此。
		* iCloud: 办公三件套，Pages, Number, Keynote。 
3. 视频
	* 课程视频：在听的存储在iCloud, 归档的视频存储到Baidu网盘。
	* 生活视频：存储在iCloud和Google. 
4. 照片
	 
	 照片会积累越来越多，但许多照片都不会去看了，但若干年后可能会搜索用。哪些事后搜索用的照片就放在Google Photo无限空间，同时Google Photo具有好的搜索引擎。哪些好照片且可公开都传到Instagram, 好照片非公开存到iCould。
	* 好而公开：Instagram(公开)
	* 好而私用：iCloud(本地连接，快速获取)
	* 事后搜索：Google(无限大且图片搜索引擎优秀)
